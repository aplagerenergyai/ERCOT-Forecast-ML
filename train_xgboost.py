...
Trains an XGBoost regressor on ERCOT price forecasting data.
...
def prepare_features_and_target(df: pd.DataFrame, target_col: str) -> Tuple[pd.DataFrame, pd.Series]:
    ...
    if missing_in_X > 0:
        logger.warning(f"Found {missing_in_X} missing values in features - will be handled by XGBoost")
...
def train_xgboost_model(X_train: pd.DataFrame, y_train: pd.Series) -> Tuple[XGBRegressor, float]:
    """
    Train an XGBoost regression model with specified hyperparameters.
    ...
    Returns:
        Tuple of (trained XGBRegressor model, training duration in seconds)
    """
    logger.info("Training XGBoost model...")
    logger.info("Hyperparameters:")
    logger.info("  n_estimators:   800")
    ...
    model = XGBRegressor(
        ...
        learning_rate=0.05,
        random_state=42,
        verbosity=0  # Suppress training output for Azure ML compatibility
    )
...
def evaluate_model(model: XGBRegressor, X_train: pd.DataFrame, y_train: pd.Series,
                   X_test: pd.DataFrame, y_test: pd.Series, training_duration: float) -> None:
    """
    ...
    Args:
        ...
        y_test: Test target
        training_duration: Time taken to train model (seconds)
    """
    ...
    # Also log to logger
    logger.info(f"Training Duration: {training_duration:.2f}s ({training_duration/60:.2f}m)")
...
def save_model(model: XGBRegressor, output_dir: str = "model_output") -> str:
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train XGBoost model for ERCOT price forecasting')
    parser.add_argument(
        '--data_path',
        type=str,
        required=True,
    ...
    try:
        ...
        # Train model (returns model and training duration)
        model, training_duration = train_xgboost_model(X_train, y_train)
        
        # Evaluate model
        evaluate_model(model, X_train, y_train, X_test, y_test, training_duration)
